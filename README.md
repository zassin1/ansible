...
The script creates an SSH key when needed, selects the configured Azure tenant and subscription, provisions the infrastructure, generates inventory, waits for SSH, runs Ansible, and prints the website URL.

## Restrict SSH before deployment

The lab defaults to `allowed_ssh_cidr = "*"`. For better security, copy the example variables file and replace `*` with your public IPv4 address followed by `/32`:
